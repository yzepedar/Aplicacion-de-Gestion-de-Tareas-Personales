  ...
  return (
    <div className="flex h-screen bg-[#f4f7fe] overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col min-w-0">
          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all 
                ${filtro === opcion.id
                ?opcion.id === 'Pendiente' ? 'bg-blue-600 text-white' : opcion.id === 'Completada' ? 'bg-green-600 text-white' : 'bg-slate-800 text-white'
        ...
        {/* Área Kanban con FILTRADO (US-06) y BÚSQUEDA (US-07) */}
                                      ...
                                      <div key={col.id} className="bg-slate-100/40 rounded-[2rem] flex flex-col min-h-[500px] border border-slate-200/50">
                                        <div className="flex justify-between items-center p-5">
                                          <div className="flex items-center gap-2">
                                            <div className={
                                              `w-2 h-2 rounded-full ${col.nombre.toUpperCase().includes('DONE') ? 'bg-green-500' : 'bg-blue-500'}`
                                            }></div>
                                                    ...
                                                    {(provided) => (
                                                      <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className="bg-white p-5 rounded-[1.8rem] shadow-sm border border-slate-200/60 transition-all group hover:shadow-md">
                                                        <div className="flex justify-between items-start mb-1">
                                                          ...
                                                          <div className="flex gap-1 ml-2">
                                                            <button
                                                              ...
                                                              className={
                                                                `p-1 rounded-md transition-all ${col.nombre.toUpperCase().includes('DONE') ? 'text-green-500 bg-green-50 opacity-100' : 'opacity-0 group-hover:opacity-100 text-slate-400 hover:text-green-500 hover:bg-green-50'}`
                                                              }
                                                            >
                                                        ...
                                                        <div className="flex justify-between items-center pt-3 border-t border-slate-50">
                                                          ...
                                                          <span className={
                                                            `text-[9px] font-black px-2 py-1 rounded-lg uppercase ${t.prioridad === 'Alta' ? 'bg-red-50 text-red-500' : t.prioridad === 'Baja' ? 'bg-green-50 text-green-500' : 'bg-orange-50 text-orange-500'}`
                                                          }>
                                                            {t.prioridad || 'Media'}
                                                          </span>
                                                        </div>
